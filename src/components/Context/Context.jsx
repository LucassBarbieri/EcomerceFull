...
export const AccountContext = createContext()                           // Crea context GOOGLE LOGIN
...
const GlobalProvider = ({ children }) => {
    ...
    const sweetWelcome = () => {
        Swal.fire({
            title: 'BETA',
            text: "Recuerda que esta aplicacion se encuentra en desarrollo.",
            icon: 'warning',
            showCancelButton: false,
            confirmButtonColor: '#440099 ',
    ...
    return (
        <>
            <GlobalContext.Provider value={{ sweetWelcome }}>
                {children}
...
export const AccountProvider = ({ children }) => {
    ...
    const [rolCheck, setRol] = useState(false);
    ...
    function setUserWhitFirebaseAndRol(user) {
    ...
    useEffect(() => {
        onAuthStateChanged(auth, (user) => {
            if (user) {
                ...
                if (!rolCheck) {
                    setUserWhitFirebaseAndRol(user);
                }
